...
public class FilterSequence extends SoundFilter {
    ...
    */
    public FilterSequence(SoundFilter[] filters) { this.filters = filters; }

    // Returns the maximum remaining size of all SoundFilters in this FilterSequence.
    public int getRemainingSize() {

        int max = 0;
        for (int i=0; i<filters.length; i++) { max = Math.max(max, filters[i].getRemainingSize()); }
        return max;
    }
    ...
    public void reset() {
        ...
        for(int i=0; i<filters.length; i++) { filters[i].reset(); }
    }

    // Filters the sound simple through each SoundFilter in this FilterSequence.
    public void filter(byte[] samples, int offset, int length) {

        for(int i=0; i<filters.length; i++) { filters[i].filter(samples, offset, length); }
    }
