...
public class TimeSmoothie {
    ...
    // How often to recalculate the frame rate
    ...
    // Time Smoothie
    public TimeSmoothie() { samples = new long[NUM_SAMPLES]; }
    ...
    public void addSample(long elapsedTime) {
        ...
        if(numSamples == samples.length) firstIndex = (firstIndex + 1) & NUM_SAMPLES_MASK;
        else numSamples++;
    }
    ...
    public long getAverage() {
        ...
        for (int i=numSamples-1; i>=0; i--) {
            ...
            // if the average period is already reached, go ahead and return the average.
            if(sum >= AVERAGE_PERIOD) Math.round((double)sum / (numSamples-i));
        }
